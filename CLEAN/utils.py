...
def get_ec_id_dict(csv_name: str) -> dict:
    ...
    id_ec = {}
    ec_id = {}
    #id_ec[P51451]=[2.7.10.2, ..]
    #ec_id[2.7.10.2]=[P51451,...]
...
def load_esm2(lookup):
    esm = format_esm(torch.load('./data/esm2_data/' + lookup + '.pt'))
    return esm.unsqueeze(0)
...
def esm2_embedding(ec_id_dict, device, dtype):
    '''
    Loading esm embedding in the sequence of EC numbers
    prepare for calculating cluster center by EC
    '''
    ...
    for ec in list(ec_id_dict.keys()):
        ids_for_query = list(ec_id_dict[ec])
        #[torch.Size([1, 1280]),torch.Size([1, 1280]),..]  与id个数相同
        esm_to_cat = [load_esm2(id) for id in ids_for_query]
        esm_emb = esm_emb + esm_to_cat
    return torch.cat(esm_emb).to(device=device, dtype=dtype)
...
def model_embedding_test_esm2(id_ec_test, model, device, dtype):
    '''
    Instead of loading esm embedding in the sequence of EC numbers
    the test embedding is loaded in the sequence of queries
    then inferenced with model to get model embedding
...
def compute_esm2_distance(train_file):
    ensure_dirs('./data/distance_map2/')
    _, ec_id_dict = get_ec_id_dict('./data/' + train_file + '.csv')
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda:0" if use_cuda else "cpu")
    dtype = torch.float32
    esm_emb = esm2_embedding(ec_id_dict, device, dtype)
    esm_dist = get_dist_map(ec_id_dict, esm_emb, device, dtype)
    
    pickle.dump(esm_dist, open('./data/distance_map2/' + train_file + '.pkl', 'wb'))
    pickle.dump(esm_emb, open('./data/distance_map2/' + train_file + '_esm.pkl', 'wb'))
...
def mutate_single_seq_ECs_forESM2(train_file):
    id_ec, ec_id =  get_ec_id_dict('./data/' + train_file + '.csv')
    single_ec = set()
    ...
    print("Number of single-seq EC number sequences need to mutate: ",len(single_id))
    print("Number of single-seq EC numbers already mutated: ", len(single_ec) - len(single_id))
    mask_sequences(single_id, train_file, train_file+'_single_seq_ECs_esm2')
    fasta_name = train_file+'_single_seq_ECs_esm2'
    return fasta_name
