...
class Triplet_dataset_with_mine_EC_ESM2(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pos = random_positive(anchor, self.id_ec, self.ec_id)
        neg = mine_negative(anchor, self.id_ec, self.ec_id, self.mine_neg)
        a = torch.load('./data/esm2_data/' + anchor + '.pt')
        p = torch.load('./data/esm2_data/' + pos + '.pt')
        n = torch.load('./data/esm2_data/' + neg + '.pt')
        return format_esm(a), format_esm(p), format_esm(n)
