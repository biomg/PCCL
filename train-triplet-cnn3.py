...
def get_dataloader(dist_map, id_ec, ec_id, args):
    ...
    train_data = Triplet_dataset_with_mine_EC_ESM2(id_ec, ec_id, negative)
...
def main():
    ...
    esm_emb = pickle.load(
        open('./data/distance_map2/' + args.training_data + '_esm.pkl',
                'rb')).to(device=device, dtype=dtype)
                
    print(esm_emb)
    print(esm_emb.shape)
    dist_map = pickle.load(open('./data/distance_map2/' + \
